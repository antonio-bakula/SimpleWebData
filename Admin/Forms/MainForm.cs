...
namespace SimpleWebDataAdmin.Forms
{
    ...
    public class MainForm : Form
    {
        ...
        protected override void OnShown(EventArgs e)
        {
            ...
            foreach (var btn in loadButtons) btn.PerformClick();
        }
        ...
        private void SetupUI()
        {
            ...
            this.Text = $"SimpleWebData - Dobrodošli, {AppState.Api.CurrentUser.Username} [(SuperUser: {isSuperAdmin})]";
        ...
        private TabPage CreateWebSitesTab()
        {
            ...
            btnAddSite.Click += async (s, e) =>
            {
                using (var modal = new Form { ClientSize = new Size(300, 140), Text = "Novi Web Site", StartPosition = FormStartPosition.CenterParent, FormBorderStyle = FormBorderStyle.FixedDialog, MaximizeBox = false, MinimizeBox = false })
                {
                    ...
                    modal.Controls.Add(lbl); modal.Controls.Add(txtCode); modal.Controls.Add(btnOk);
        ...
        private TabPage CreateUsersTab()
        {
            ...
            {
                var sites = await AppState.Api.GetAsync<List<WebSite>>("/api/superadmin/websites");
                if (sites == null || sites.Count == 0) { MessageBox.Show("Nema kreiranih Web Site-ova!"); return; }
                ...
                using (var modal = new Form { ClientSize = new Size(300, 300), Text = "Novi Korisnik", StartPosition = FormStartPosition.CenterParent, FormBorderStyle = FormBorderStyle.FixedDialog, MaximizeBox = false, MinimizeBox = false })
                {
                    ...
                    modal.Controls.Add(lblUser); modal.Controls.Add(txtUser);
                    modal.Controls.Add(lblPass); modal.Controls.Add(txtPass);
                    modal.Controls.Add(lblSite); modal.Controls.Add(cmbSite);
                    modal.Controls.Add(chkSuper); modal.Controls.Add(btnOk);
                    modal.AcceptButton = btnOk;
        ...
        private void HideTechnicalColumns(DataGridView grid)
        {
            grid.DataBindingComplete += (s, e) =>
            {
                ...
                if (grid.Columns["WebSiteId"] != null) grid.Columns["WebSiteId"].Visible = false;
                if (grid.Columns["PhotoGalleryId"] != null) grid.Columns["PhotoGalleryId"].Visible = false;
                if (grid.Columns["PageId"] != null) grid.Columns["PageId"].Visible = false;
                if (grid.Columns["FacilityId"] != null) grid.Columns["FacilityId"].Visible = false;
            };
        }

        private TabPage CreateStandardTab<T>(string title, string apiUrl)
        {
            var tab = new TabPage(title);
            tab.BackColor = Color.White;
            
            var flowTop = new FlowLayoutPanel { Dock = DockStyle.Top, AutoSize = true, Padding = new Padding(10) };
            var btnLoad = new Button { Text = "Osvježi", AutoSize = true, MinimumSize = new Size(150, 40), BackColor = Color.LightGray, Margin = new Padding(5) };
            var btnDelete = new Button { Text = "Obriši odabrano", AutoSize = true, MinimumSize = new Size(150, 40), BackColor = Color.MistyRose, Margin = new Padding(5) };
            ...
            btnLoad.Click += async (s, e) =>
            {
                var data = await AppState.Api.GetAsync<List<T>>(apiUrl);
            ...
            btnDelete.Click += async (s, e) =>
            {
                ...
                var id = (int)idProp.GetValue(row.DataBoundItem)!;
                
                if (MessageBox.Show("Jeste li sigurni?", "Potvrda brisanja", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    await AppState.Api.DeleteAsync($"{apiUrl}/{id}");
                    btnLoad.PerformClick();
                }
            };
            ...
            // Dodaj ga u globalnu listu za prvo učitavanje
            loadButtons.Add(btnLoad);
            tab.Tag = btnLoad;
        ...
        private TabPage CreatePagesTab()
        {
            ...
            flowLeft.Controls.Add(btnLoad); flowLeft.Controls.Add(btnAddPage); flowLeft.Controls.Add(btnDelPage);
            ...
            gLeft.Controls.Add(gridPages); gLeft.Controls.Add(flowLeft);
            ...
            flowRight.Controls.Add(btnAddText); flowRight.Controls.Add(btnDelText);
            ...
            gRight.Controls.Add(gridTexts); gRight.Controls.Add(flowRight);
            ...
            btnAddText.Click += async (s, e) =>
            {
                ...
                using (var modal = new Form { ClientSize = new Size(400, 300), Text = "Novi Tekst", StartPosition = FormStartPosition.CenterParent, FormBorderStyle = FormBorderStyle.FixedDialog, MaximizeBox = false, MinimizeBox = false })
                {
                    ...
                    modal.Controls.Add(lblCode); modal.Controls.Add(txtCode);
                    modal.Controls.Add(lblContent); modal.Controls.Add(txtContent);
        ...
        private TabPage CreateFacilitiesTab()
        {
            ...
            flowLeft.Controls.Add(btnLoad); flowLeft.Controls.Add(btnAddFac); flowLeft.Controls.Add(btnDelFac);
            ...
            gLeft.Controls.Add(gridFac); gLeft.Controls.Add(flowLeft);
            ...
            flowRight.Controls.Add(btnAddDates); flowRight.Controls.Add(btnDelDate);
            ...
            gRight.Controls.Add(gridRes); gRight.Controls.Add(flowRight);
            ...
            gridFac.DataBindingComplete += (s, e) => {
                if (gridFac.Columns["PhotoGalleryId"] != null) gridFac.Columns["PhotoGalleryId"].Visible = false;
                if (gridFac.Columns["Status"] != null) gridFac.Columns["Status"].Visible = false;
            };
            gridRes.DataBindingComplete += (s, e) => {
                if (gridRes.Columns["Status"] != null) gridRes.Columns["Status"].Visible = false;
                if (gridRes.Columns["FacilityId"] != null) gridRes.Columns["FacilityId"].Visible = false;
            };
            ...
            gridFac.SelectionChanged += async (s, e) =>
            {
                if (gridFac.SelectedRows.Count > 0 && gridFac.DataSource != null)
                {
                    ...
                    if (f != null)
                    {
                        ...
                        if (gridRes.Columns["Date"] != null) gridRes.Columns["Date"].ReadOnly = true; 
                    }
                }
                else
                {
            ...
            btnAddFac.Click += async (s, e) =>
            {
                using (var fModal = new Form { Width = 300, Height = 170, Text = "Novi Objekt", StartPosition = FormStartPosition.CenterParent })
                {
                    ...
                    fModal.Controls.Add(lbl); fModal.Controls.Add(txtCode); fModal.Controls.Add(btnOk);
            ...
            btnAddDates.Click += async (s, e) =>
            {
                ...
                using (var modal = new Form { ClientSize = new Size(250, 240), Text = "Dodaj Datume", StartPosition = FormStartPosition.CenterParent, FormBorderStyle = FormBorderStyle.FixedDialog, MaximizeBox = false, MinimizeBox = false })
                {
                    ...
                    modal.Controls.Add(lblOd); modal.Controls.Add(dtpOd);
                    modal.Controls.Add(lblDo); modal.Controls.Add(dtpDo);
                    ...
                    if (modal.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (start > end) { MessageBox.Show("Datum DO mora biti >= OD!"); return; }
        ...
        private TabPage CreateGalleryTab()
        {
            ...
            btnAddGal.Click += async (s, e) => {
                ...
                if(res != null) { btnLoad.PerformClick(); }
            };

            btnDelGal.Click += async (s, e) => {
                if (gridGalleries.SelectedRows.Count == 0) return;
                var g = (PhotoGallery)gridGalleries.SelectedRows[0].DataBoundItem;
            ...
            btnUpload.Click += async (s, e) =>
            {
                ...
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    ...
                    if (success) { MessageBox.Show("Dodano!"); btnLoad.PerformClick(); }
                }
            };

            btnChangeImg.Click += async (s,e) => {
                 if (gridPhotos.SelectedRows.Count == 0) return;
                 var p = (Photo)gridPhotos.SelectedRows[0].DataBoundItem;
                 using var ofd = new OpenFileDialog { Filter = "Image Files|*.jpg;*.jpeg;*.png;*.svg" };
                 if (ofd.ShowDialog() == DialogResult.OK)
                 {
                     bool success = await AppState.Api.UpdatePhotoAsync(p.Id, ofd.FileName, p.AltText);
                     if (success) { MessageBox.Show("Izmijenjeno!"); btnLoad.PerformClick(); }
                 }
            };
        ...
        private TabPage CreateApiKeyTab()
        {
            ...
            btnGenKey.Click += async (s, e) =>
            {
                ...
                for (int i = 0; i < domains.Length; i++) domains[i] = domains[i].Trim();
